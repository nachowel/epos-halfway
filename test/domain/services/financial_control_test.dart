  ...
  group('Financial control phase 6', () {
      ...
      () async {
        ...
        expect(
          ...
          containsAll(<String>[
            ...
            'payment_taken',
            'payment_refunded',
            'stale_draft_discarded',
            'cashier_preview_close',
            'shift_reconciliation_recorded',
            'admin_final_close',
          ]),
        );
        final paymentEntry = entries.firstWhere(
          (entry) => entry.actionType == 'payment_taken',
        );
        expect(paymentEntry.actorId, harness.admin.id);
        expect(paymentEntry.actorRole, 'admin');
        expect(paymentEntry.entityType, 'transaction');
        expect(paymentEntry.entityId, 'audit-flow');
        final reconciliationEntry = entries.firstWhere(
          (entry) => entry.actionType == 'shift_reconciliation_recorded',
        );
        expect(reconciliationEntry.entityType, 'shift_reconciliation');
        expect(
          reconciliationEntry.metadata['counted_cash_source'],
    ...
    test(
      ...
      () async {
        ...
        final AuditLogRecord reconciliationEntry = (await harness
                .auditLogRepository
                .listRecent(limit: 20))
            .firstWhere(
              (AuditLogRecord entry) =>
                  entry.actionType == 'shift_reconciliation_recorded',
            );
