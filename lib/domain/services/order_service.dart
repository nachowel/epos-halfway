...
import '../models/draft_order_policy.dart';
...
class OrderService {
  ...
  Future<void> sendOrder({
    ...
    await _transactionRepository.runInTransaction(() async {
      ...
      await _auditLogService.recordAction(
        ...
        actor: currentUser,
        metadata: <String, Object?>{'transaction_id': transactionId},
      );
      _logger.audit(
  ...
  Future<void> discardDraft({
    ...
    final String discardActionType = DraftOrderPolicy.isStale(transaction)
        ? 'stale_draft_discarded'
        : 'draft_discarded';
    await _auditLogService.recordAction(
      entityType: 'transaction',
      entityId: transaction.uuid,
      actionType: discardActionType,
      actor: currentUser,
      metadata: <String, Object?>{'transaction_id': transactionId},
    );
    _logger.audit(
  ...
  Future<List<OpenOrderSummary>> getOrderSummariesByShift(int shiftId) async {
    final List<Transaction> orders = await getOrdersByShift(shiftId);
